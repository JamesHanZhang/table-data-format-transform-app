...
class DiffTransBatchFiles(FormatTransformation):
    ...
    def run_single_file_transformation(self, params_set, overwrite, input_path, input_file, based_on_activation):
        ...
        self.reset_params(params_set, overwrite, input_path=input_path, input_file=input_file, output_file=output_file,
    ...
    def diff_trans_batch_files(self, params_set="", overwrite=False, import_type:str="", input_path="", output_path="", based_on_activation:bool=None):
        """
        :param params_set: 复用同一个params_set的设置
        :param overwrite: True: 使用_params_setting.py的设置并覆盖.json参数文件; False: 使用.json参数文件
        ...
        :param based_on_activation: 激活模式选定
        ...
        self.reset_params(params_set, overwrite, input_path=input_path, output_path=output_path)
        # 执行程序
        ...
        for input_path in tqdm(path_file_pairs.keys(),position=True,leave=True,desc="different input paths (including child paths) running..."):
            for input_file in tqdm(path_file_pairs[input_path],position=True,leave=True,desc="differnet file transformation running..."):
                self.run_single_file_transformation(params_set, overwrite, input_path, input_file, based_on_activation)
        end_program(start_time)
        return
    
if __name__ == "__main__":
    dtbf = DiffTransBatchFiles()
    dtbf.diff_trans_batch_files(overwrite=True,import_type='.csv',based_on_activation=True)