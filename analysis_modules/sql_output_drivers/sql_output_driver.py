...
class SqlOutputDriver(DfOutputDriver):
    ...
    def create_table_insert_sql(self, df: pd.DataFrame, full_output_path: str, overwrite: bool, base_num: int):
        ...
        for row in tqdm(range(row_num), position=0, leave=True, desc="process for table insert sql creation..."):
            ...
            if (row + 1) % self.commit_range == 0:
                ...
                self.iom.store_file(full_output_path, content=collected_sql, encoding=self.output_encoding,
                                    overwrite=False)
                collected_sql = ""
    ...
    def store_df_as_sql(self, df: pd.DataFrame, table_name="", output_path="", output_encoding="",
                        overwrite: bool = None,
                        if_sep: bool = None, only_one_chunk: bool = None, table_comment="",
        ...
        base_num = self.chunksize * (
            0 if type(chunk_no) is str and chunk_no == "" else chunk_no) + self.original_base_num
        
        insert_file = self.decide_sep_or_add(self.table_name + "_table_insert", self.if_sep, self.only_one_chunk,
                                             chunk_no)
    ...
    def sep_df_as_multi_sql(self, df: pd.DataFrame, table_name="", output_path="", output_encoding="",
                            overwrite: bool = None,
