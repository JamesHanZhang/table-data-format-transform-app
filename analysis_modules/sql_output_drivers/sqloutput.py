...
class SqlOutput:
    ...
    def init_output_params(self, output_params):
        self.database = output_params.sql_output_params.database
        self.if_sep = output_params.if_sep
        if self.database == "Oracle":
            self.sql_out_driver = OracleOutputDriver(output_params)
        elif self.database == "MySql":
            self.sql_out_driver = MySqlOutputDriver(output_params)
        elif self.database == "GBase":
            self.sql_out_driver = GBaseOutputDriver(output_params)
        elif self.database == "PostgreSQL":
            self.sql_out_driver = PostgreSqlOutputDriver(output_params)
        elif self.database == "SqlServer":
            self.sql_out_driver = SqlServerOutputDriver(output_params)
        else:
            raise TypeError("[TypeError] the database you choose didn't follow the rule in the list of database choices.")
    
    def output_as_sql(self, df: pd.DataFrame, output_params: OutputParams, table_name="", output_path="",
                      output_encoding="", overwrite=None, chunk_no="", base_num=0):
        self.init_output_params(output_params)
        self.sql_out_driver.store_df_as_sql(df, output_params, table_name, output_path, output_encoding, overwrite, chunk_no, base_num)
        return
    
    def output_as_sql_in_pieces(self, df: pd.DataFrame, output_params: OutputParams, table_name="", output_path="",
                                output_encoding="", only_one_chunk:bool=None):
        self.init_output_params(output_params)
        self.sql_out_driver.sep_df_as_multi_sql(df, table_name, output_path, output_encoding, only_one_chunk)
        return
    
    def output_as_sql_control(self, df: pd.DataFrame, output_params: OutputParams, table_name="", output_path="",
                              output_encoding="", if_sep:bool=None, overwrite=None, chunk_no:int="", base_num=0,
                              only_one_chunk=None):
        """
        ...
        if if_sep is True:
            self.output_as_sql_in_pieces(df, output_params, table_name, output_path, output_encoding, only_one_chunk)
        else:
            self.output_as_sql(df, output_params, table_name, output_path, output_encoding, overwrite, chunk_no, base_num)
        return
