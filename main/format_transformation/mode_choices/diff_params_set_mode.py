...
def init_diff_params_mode():
    ...
    while True:
        try:
            ...
            log = f"您目前采用的模式为: \n{ModeChoicesDesc.DIFF_PARAMS_MODE.value}\n"
            SysLog.show_log(log)
            
            params_sets = init_params_creation_list()
            output_mode = init_output_mode()
            
            print(f"\n######################## 您所输入的参数如下, 请重新确认 ################################\n\n"
                  f"您所输入的多个参数表名为: {str(params_sets)}\n"
                  f"共计有{len(params_sets)}个参数表待搭建;\n"
...
def run_diff_params_mode():
    params_sets, output_mode = init_diff_params_mode()
